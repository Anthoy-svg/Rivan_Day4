...
conf t
 ...
 int g3
  ...
  exit
 int g4
  ip add 10.11.1.11 255.255.255.0
  no shut
  exit
 !
...
### Task 1: Prevent traffic only from VLAN 100 (CUCM) to reach UTM-PH (10.11.1.11)
~~~
!@UTM-PH
config t
...
### Task 2: Prevent traffic coming from the PC (10.11.1.10) and the EDGE Router from reaching UTM-PH (10.11.1.11)
~~~
!@UTM-PH
config t
...
### Task 3: Allow the PC (10.11.1.10) to access HTTP, SSH, and ICMP of UTM-PH (10.11.1.11)
...
!@UTM-PH
config t
 ip host www.bet11.com 10.11.1.11
 service finger
 ...
 telephony-service
  ...
  max-ephones 5
  max-dn 20
  ip source-address 10.11.1.11 port 2000
  exit
 voice service voip
 ...
 voice register global
  mode cme
  source-address 10.11.1.11 port 5060
  max-dn 12
  max-pool 12
...
!@EDGE-11
conf t
 ip nat inside source static tcp  10.11.1.11  6969  200.0.0.11  3306
 end
~~~
...
conf t
 ...
 ip access-list extended NAT-POLICY
  deny ip 10.11.0.0 0.0.255.255 10.11.0.0 0.0.255.255
